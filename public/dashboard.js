...
export function initDashboard(apiEndpoints) {
    const app = createApp({
        ...
        computed: {
            filteredRoutes() {
                ...
                if (this.selectedBundle !== 'all') {
                    filtered = filtered.filter(route =>
                        route.bundle === this.selectedBundle
                    );
                }
        ...
        methods: {
            async fetchData() {
                try {
                    const [routes, grouped, tree, stats, analytics, sqlData, perfIssues] = await Promise.all([
            ...
            initRouteGraph() {
                ...
                const svg = d3.select('#route-graph-container')
                    .append('svg')
                    .attr('width', width)
                    .attr('height', height);
                ...
                node.append('circle')
                    ...
                    .style('fill', d => d.data.isEndpoint ? this.getMethodColor(d.data.methods?.[0]) : 'var(--accent-primary)')
                    ...
                    .on('mouseout', (event, d) => {
                        d3.select(event.currentTarget).transition().duration(200).attr('r', d.data.isEndpoint ? 6 : 8);
                ...
                const badgeGroup = node.filter(d => d.data.isEndpoint && d.data.methods)
                    .append('g')
            ...
            resetGraphZoom() {
                ...
                svg.transition().duration(750).call(
                    d3.zoom().transform,
                    d3.zoomIdentity.translate(40, 40)
                );
            },
            exportGraphAsSVG() {
                    ...
                    <div class="container header-content">
                        <div class="header-left">
                            ...
                            <p class="dashboard-subtitle">
                                Visual Route Performance Monitor - Track, analyze, and optimize your Symfony routes over time. See which routes are slow, which have N+1 queries, and compare performance across your entire application with user friendly visual route explorer
                            </p>
                        </div>
